...
# ============================================================================
# _create_env_file (skipped tests — preserved for future MLflow resolver work)
# ============================================================================
...
def test_create_env_file_docker_mode_sets_workspace_to_container_path(
    base_config: PipelineConfig, secrets: DummySecrets
):
...
def test_create_env_file_exec_fails_returns_err(base_config: PipelineConfig, secrets: DummySecrets):
    manager = TrainingDeploymentManager(config=base_config, secrets=secrets)
    manager.set_workspace(workspace_path="/workspace")
    ...
    result = manager._launcher._create_env_file(ssh_client, context=None)
    assert result.is_err()
