...
def test_tui_command_accepts_log_level_and_configures_logger(cli_runner, tmp_path):
    ...
    fake_app_cls.return_value.run.return_value = None

    with (
        patch("src.utils.logger.set_log_level") as mock_set_log_level,
        patch.dict("sys.modules", {"src.tui.apps": types.SimpleNamespace(RyotenkaiApp=fake_app_cls)}),
    ):
        result = cli_runner.invoke(app, ["tui", str(tmp_path / "runs" / "run_1"), "--log-level", "DEBUG"])
...
def test_tui_command_rejects_unknown_log_level(cli_runner):
    ...
    assert result.exit_code != 0
    assert "log level must be INFO or DEBUG" in result.output
