...
from src.tui.adapters.state import discover_run_dirs, load_pipeline_state

_TUI_LOG = logging.getLogger("ryotenkai.tui.delete_backend")
...
class TuiDeleteBackend:
    ...
    def _delete_mlflow_for_run(self, run_dir: Path, deleted_ids: list[str]) -> list[DeleteIssue]:
        ...
        try:
            ...
            _TUI_LOG.exception("MLflow delete failed for %s", run_dir)
    ...
    def _safe_get_run(client, run_id: str):
        try:
            ...
            if TuiDeleteBackend._is_missing_run_error(exc):
                return None
            raise
