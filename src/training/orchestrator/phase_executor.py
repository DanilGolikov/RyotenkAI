...
class PhaseExecutor:
    ...
    def execute(
        ...
        *,
        ...
        2. [Adapter cache] If enabled and not upstream_retrained: check HF for cached adapter
           - Cache hit  → mark skipped, return Ok(loaded_model)
           - Cache miss → proceed to training
        3. Mark phase started
        4. Create strategy (for data preparation)
        5. Load and validate dataset
        6. Prepare dataset with strategy
        7. Create trainer
        8. Run training (with shutdown check)
        9. Save checkpoint
        10. Extract metrics
        11. Mark phase completed
        12. [Adapter cache] If enabled: upload adapter to HF (soft-fail)
        ...
        Args:
            ...
            upstream_retrained: If True, skip cache hit check (upstream phase was retrained)
        ...
        # 1. [ADAPTER CACHE] CHECK HF CACHE BEFORE TRAINING
        ...
        try:
            ...
            # 16. [ADAPTER CACHE] UPLOAD TRAINED ADAPTER TO HF (soft-fail)
    ...
    def _compute_dataset_fingerprint(dataset_config: Any) -> str:
        ...
        if source_type == "local":
            ...
            try:
                ...
                logger.debug(f"[PE:FINGERPRINT_HF_WARN] could not fetch dataset_info for {train_id}: {e}")
                commit_sha = ""
    ...
    def _try_adapter_cache_hit(
        ...
        # 1. Lightweight tag existence check (no download)
        try:
            api = HfApi()
            ...
            logger.warning(
                f"[PE:ADAPTER_CACHE_CHECK_WARN] phase={phase_idx}: could not list refs for {repo_id}: {e}. "
        ...
        # 2. Cache hit — download adapter
        logger.info(
            f"   🎯 Adapter cache hit: phase {phase_idx} ({phase.strategy_type}), "
    ...
    def _upload_adapter_to_cache(
        ...
        logger.info(
            f"   ⬆️  Uploading adapter to cache: {repo_id}@{expected_tag} ..."
        )
        ...
        try:
            ...
            buffer.save_state()
            logger.info(
                f"   ✅ Adapter cached: {repo_id}@{expected_tag}"
            )
        except Exception as e:
            err_msg = str(e)
