"""
Average Length validation plugin.

Checks average text length of samples.
"""
...
class AvgLengthValidator(ValidationPlugin):
    """
    Validates average text length in dataset.

    Params:
        min (int): Minimum average length (default: 50)
        max (int): Maximum average length (default: 8192)
        sample_size (int): For large/streaming datasets (default: 10000)

    Recommendations on failure:
        - Check if samples are truncated
    ...
    def get_description(cls) -> str:
        return "Checks average text length across dataset examples"
    ...
    def validate(
        ...
        if avg_length < min_length * _NEAR_WARNING_FACTOR:
            warnings.append(f"Average length is close to the minimum: {avg_length:.1f} (min: {min_length})")
