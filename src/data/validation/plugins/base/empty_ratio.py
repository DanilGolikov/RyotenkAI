"""
Empty Ratio validation plugin.

Checks ratio of empty or very short samples.
"""
...
class EmptyRatioValidator(ValidationPlugin):
    """
    Validates ratio of empty/very short samples.

    Params:
        max_ratio (float): Maximum allowed ratio (default: 0.1 = 10%)
        min_chars (int): Minimum chars to not be considered empty (default: 10)
        sample_size (int): For large/streaming datasets (default: 10000)

    Recommendations on failure:
        - Clean dataset (remove empty samples)
    ...
    def get_description(cls) -> str:
        return "Checks the ratio of empty or very short examples"

    def validate(
        self,
        dataset: Dataset | IterableDataset,
    ) -> ValidationResult:
        """Check ratio of empty samples."""
