...
class HelixQLPreferenceSemanticsValidator(ValidationPlugin):
    ...
    def get_description(cls) -> str:
        return (
            "Checks that the chosen/rejected pair is domain-consistent and chosen is objectively better than rejected"
        )

    def _validate_contract(self) -> None:
        timeout_seconds = self.params.get("timeout_seconds")
        if timeout_seconds is None or int(timeout_seconds) <= 0:
            raise ValueError("helixql_preference_semantics requires positive params.timeout_seconds")
        max_error_examples = self._max_error_examples()
        if max_error_examples < -1:
            raise ValueError("helixql_preference_semantics requires params.max_error_examples >= -1")
    ...
    def _validate_pair(self, *, sample: Mapping[str, Any]) -> str | None:
        ...
        schema_text = extract_query_text(sample.get("schema_context")) or extract_schema_block(prompt_text)
        reference_answer = extract_query_text(sample.get("reference_answer") or sample.get("expected_answer"))
