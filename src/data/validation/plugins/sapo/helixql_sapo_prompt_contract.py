...
class HelixQLSAPOPromptContractValidator(ValidationPlugin):
    ...
    def get_description(cls) -> str:
        return "Checks that the sample is suitable for prompt-only GRPO/SAPO mode"

    def _validate_contract(self) -> None:
        max_error_examples = self._max_error_examples()
        if max_error_examples < -1:
            raise ValueError("helixql_sapo_prompt_contract requires params.max_error_examples >= -1")
