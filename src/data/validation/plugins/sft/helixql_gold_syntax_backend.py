...
class HelixQLGoldSyntaxBackendValidator(ValidationPlugin):
    ...
    def get_description(cls) -> str:
        return "Validates gold HelixQL via Helix CLI compile (syntax + analysis, no cargo)"

    def _validate_contract(self) -> None:
        timeout_seconds = self.params.get("timeout_seconds")
        if timeout_seconds is None or int(timeout_seconds) <= 0:
            raise ValueError("helixql_gold_syntax_backend requires positive params.timeout_seconds")
        max_error_examples = self._max_error_examples()
        if max_error_examples < -1:
            raise ValueError("helixql_gold_syntax_backend requires params.max_error_examples >= -1")
