...
class HelixQLGeneratedSyntaxBackendPlugin(EvaluatorPlugin):
    name = "helixql_generated_syntax_backend"
    priority = 8
    ...
    def get_description(cls) -> str:
        return "Validates model_answer via Helix CLI compile (syntax + analysis, no cargo)"

    def _validate_contract(self) -> None:
        timeout_seconds = self._param("timeout_seconds")
        if timeout_seconds is None or int(timeout_seconds) <= 0:
            raise ValueError("helixql_generated_syntax_backend requires positive params.timeout_seconds")
        min_valid_ratio = float(self._threshold("min_valid_ratio", 0.8))
        if not (0.0 <= min_valid_ratio <= 1.0):
            raise ValueError("helixql_generated_syntax_backend thresholds.min_valid_ratio must be in [0, 1]")
    ...
    def evaluate(self, samples: list[EvalSample]) -> EvalResult:
        if not samples:
            return EvalResult(plugin_name=self.name, passed=True, metrics={"valid_ratio": 1.0}, sample_count=0)
        ...
        for idx, sample in enumerate(samples):
            schema_text = extract_query_text(sample.metadata.get("schema_context")) or extract_schema_block(
                sample.question
            )
        ...
        if not passed:
            errors.append(
                f"valid_ratio={valid_ratio:.2%} is below threshold {min_valid_ratio:.2%} ({valid_count}/{total} passed)"
            )
