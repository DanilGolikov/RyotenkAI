...
def test_load_restart_point_options_maps_plain_dicts_to_dataclass(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("ryotenkai_control.pipeline.launch.restart_options.load_config", lambda _path: config_obj)
...
def test_load_restart_point_options_propagates_load_config_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("ryotenkai_control.pipeline.launch.restart_options.load_config", lambda _path: (_ for _ in ()).throw(RuntimeError("bad config")))
...
def test_load_restart_point_options_propagates_restart_point_query_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("ryotenkai_control.pipeline.launch.restart_options.load_config", lambda _path: MagicMock())
    monkeypatch.setattr(
