...
class BootstrapResult:
    ...
    # Narrow MLflow lifecycle stack -- the single source of truth for
    # the MLflow attempt lifecycle. The orchestrator owns the timing:
    # open/close/preflight all flow through these collaborators.
...
class PipelineBootstrap:
    ...
    def build(
        ...
        *,
        ...
        # MLflow attempt lifecycle. ``None`` values mean the orchestrator's
        # ``_setup_mlflow_for_attempt`` / ``_ensure_mlflow_preflight``
        # degrade to no-ops (e.g. when no ``tracking_uri`` is configured).
        (
            mlflow_transport,
            mlflow_run_query,
