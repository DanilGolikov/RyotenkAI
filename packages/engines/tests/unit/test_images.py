...
def _fake_manifest(
    *,
    ...
    actually reads."""
    m = MagicMock()
    m.engine = _FakeEngineSpec(id=engine_id, version=version)
    m.image = _FakeImageSpec(default=explicit_image) if explicit_image else None
    return m
...
class TestOverrideChain:
    ...
    def test_provider_override_wins_over_manifest(self) -> None:
        ...
        prov_obj = MagicMock()
        prov_obj.image = "provider/override:1"
        out = resolve_image(
            engine_id="vllm",
            manifest=manifest,
