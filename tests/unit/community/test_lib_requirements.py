...
def _libs_by_id(community_root: Path) -> dict[str, LibManifest]:
    ...
    from ryotenkai_community.libs import libs_root_for, load_libs
...
class TestCrossCheck:
    ...
    def test_toml_only_lib_diff(self, with_alpha_lib: Path) -> None:
        ...
        with pytest.raises(
            ValueError, match=r"manifest but missing from REQUIRED_LIBS.*beta"
        ):
            load_plugins(
                "validation",
                root=community_root,
...
class TestVersionSatisfaction:
    ...
    def test_constraint_unsatisfied(self, with_alpha_lib: Path) -> None:
        ...
        with pytest.raises(ValueError, match=r"lib 'alpha' is at version 1.0.0"):
            load_plugins(
                "validation",
                root=community_root,
                strict=True,
