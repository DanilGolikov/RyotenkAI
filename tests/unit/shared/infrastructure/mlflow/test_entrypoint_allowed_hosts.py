...
# Pre-existing legacy debt: the entrypoint script invokes `mlflow db upgrade`
# before printing the argv we care about. The test rigs PATH=/usr/bin:/bin
# for the subprocess, which strips the venv's mlflow CLI even when one is
# installed globally — there's no way for the dry-run to complete in this
# minimal-PATH harness. Skip when the binary isn't reachable from the
# restricted PATH the test itself uses.
_MLFLOW_REACHABLE_IN_TEST_PATH = any(
    shutil.which("mlflow", path=p) for p in ("/usr/bin", "/bin")
)

pytestmark = [
    pytest.mark.unit,
    pytest.mark.skipif(
        not _MLFLOW_REACHABLE_IN_TEST_PATH,
        reason=(
            "Pre-existing: entrypoint dry-run sets PATH=/usr/bin:/bin and "
            "needs the mlflow CLI on that PATH; not available in this env."
        ),
    ),
]
