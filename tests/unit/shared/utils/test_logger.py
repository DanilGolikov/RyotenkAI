...
def test_stage_logging_context_empty_stage_name_raises(
    ...
    with pytest.raises(ValueError, match="stage_name must be non-empty"):  # noqa: SIM117
...
def test_stage_filter_captures_threadpool_record_via_global_fallback(
    ...
    with stage_logging_context("Dataset Validator", layout), ThreadPoolExecutor(max_workers=1) as executor:
...
def test_active_stage_survives_exception_in_stage(
    ...
    with pytest.raises(RuntimeError, match="boom"), stage_logging_context("Dataset Validator", layout):
