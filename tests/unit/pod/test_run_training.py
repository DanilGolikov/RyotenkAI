...
# DEAD (xfail audit 2026-05-12): removed TestTrainingFileHandler — production
# removed ``_install_training_file_handler`` and ``RYOTENKAI_TRAINING_LOG_PATH``
# during Phase B packagization. The per-attempt log file is now managed by
# Supervisor capturing trainer stdio. Behaviour is covered by the
# Supervisor stdio-capture suite, not by this trainer-internal API.
