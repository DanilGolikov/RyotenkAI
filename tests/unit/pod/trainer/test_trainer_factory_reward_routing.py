...
# FIXABLE marker removed 2026-05-12 (xfail audit): `_make_pipeline_config`
# now deletes `cfg.training.adapter` so production skips the PEFT gate.
# ---------------------------------------------------------------------------
# Shared stubs
# ---------------------------------------------------------------------------
...
def _make_pipeline_config(*, training_type: str = "full") -> MagicMock:
    ...
    # Production now gates PEFT config creation on `hasattr(config.training,
    # "adapter")`. The MagicMock auto-creates that attribute, which trips a
    # ValidationError inside create_peft_config when it tries to inspect
    # `adapter.kind.lower()`. Delete the auto-attribute so the
    # production code skips the PEFT branch entirely (these tests
    # exercise reward routing, not adapter wiring).
    del cfg.training.adapter
    return cfg
