...
def _stamp_manifest_on_providers() -> None:
    """Ensure manifest ClassVars are attached on RunPodProvider before tests run.

    ``ProviderRegistry`` stamps these on first ``create_training()``
    call, but this file builds providers via ``__new__`` (skipping the
    registry). When the file is run in isolation, the ClassVars are
    None and ``ProviderBase.get_capabilities`` raises ``RuntimeError``.
    Calling ``capabilities()`` is a no-op when manifest data is already
    present (idempotent) and stamps it otherwise — same operation the
    registry performs at production startup.
    """
    registry = get_registry()
    # Touching capabilities + resolving the class via the public API
    # stamps the manifest ClassVars as a side-effect of class resolution.
    for provider_id in ("runpod",):
        try:
            registry.capabilities(provider_id)
            # Force class resolution to trigger _attach_manifest_metadata.
            registry._resolve_class(provider_id, role_key="training")  # type: ignore[attr-defined]
...
# now takes a single ProviderContext argument (post Phase B packagization).
# Every ``[single_node]`` parametrization below blows up at _mk_single_node().
# Marked with strict=True so when the helper is ported, the markers flip back
# to failures and force themselves to be removed.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(_mk_single_node, False, id="single_node", marks=_SINGLENODE_XFAIL),
    ],
)
...
@pytest.mark.parametrize(
    "factory",
    [
        pytest.param(_mk_runpod, id="runpod"),
        pytest.param(_mk_single_node, id="single_node", marks=_SINGLENODE_XFAIL),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(_mk_single_node, "local_disk", id="single_node", marks=_SINGLENODE_XFAIL),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(_mk_single_node, "/workspace", id="single_node", marks=_SINGLENODE_XFAIL),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(_mk_single_node, True, id="single_node", marks=_SINGLENODE_XFAIL),
    ],
)
