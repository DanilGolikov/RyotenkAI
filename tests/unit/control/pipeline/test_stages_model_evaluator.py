...
def _mk_cfg(*, eval_enabled: bool = True) -> MagicMock:
    ...
    # (typed engine config discriminator); previously it read the raw
    # string. Wire a MagicMock with the ``kind`` attribute so the
    # production path resolves cleanly.
    cfg.inference.engine = MagicMock(kind="vllm")
