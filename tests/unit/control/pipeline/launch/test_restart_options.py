...
def test_load_restart_point_options_maps_plain_dicts_to_dataclass(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "ryotenkai_control.pipeline.launch.restart_options.resolve_config_path_for_run",
        lambda run_dir, provided_config_path=None: provided_config_path,
    )
    monkeypatch.setattr("ryotenkai_control.pipeline.launch.restart_options.load_pipeline_config", lambda _path: config_obj)
    monkeypatch.setattr("ryotenkai_control.pipeline.launch.restart_options.list_restart_points", lambda run_dir, config: restart_points)
...
def test_load_restart_point_options_propagates_load_config_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("ryotenkai_control.pipeline.launch.restart_options.load_pipeline_config", lambda _path: (_ for _ in ()).throw(RuntimeError("bad config")))
...
def test_load_restart_point_options_propagates_restart_point_query_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("ryotenkai_control.pipeline.launch.restart_options.load_pipeline_config", lambda _path: MagicMock())
    monkeypatch.setattr(
        "ryotenkai_control.pipeline.launch.restart_options.list_restart_points",
...
def test_pick_default_launch_mode_returns_restart_when_state_load_fails(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "ryotenkai_control.pipeline.launch.restart_options.PipelineStateStore.load",
...
def test_validate_resume_run_allows_provider_only_hash_drift_when_model_dataset_hash_matches(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("ryotenkai_control.pipeline.launch.restart_options.load_pipeline_config", lambda _path: config_obj)
    monkeypatch.setattr(
        "ryotenkai_control.pipeline.launch.restart_options.compute_config_hashes",
        lambda _config: {
