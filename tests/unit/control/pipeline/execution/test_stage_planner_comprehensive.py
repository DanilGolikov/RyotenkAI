...
from ryotenkai_control.pipeline.execution.stage_planner import (
    StagePlanner,
...
class TestDependencyErrors:
    ...
    def test_health_check_handles_socket_timeout(self) -> None:
        with patch(
            "ryotenkai_control.pipeline.execution.stage_planner.urlopen", side_effect=socket.timeout("slow")
        ):
            assert is_inference_runtime_healthy({"endpoint_url": "http://x"}) is False

    def test_health_check_handles_value_error(self) -> None:
        with patch(
            "ryotenkai_control.pipeline.execution.stage_planner.urlopen", side_effect=ValueError("bad url")
        ):
            assert is_inference_runtime_healthy({"endpoint_url": "http://x"}) is False
    ...
    def test_prereq_model_evaluator_with_unhealthy_runtime_failing_urlopen(
        ...
        with patch(
            "ryotenkai_control.pipeline.execution.stage_planner.urlopen",
...
class TestHealthCheckPositiveNegative:
    def test_positive_200_ok(self) -> None:
        ...
        with patch("ryotenkai_control.pipeline.execution.stage_planner.urlopen", return_value=resp):
            assert is_inference_runtime_healthy({"endpoint_url": "http://x"}) is True
