...
def test_build_ssh_opts_alias_mode():
    ssh_client = SimpleNamespace(ssh_base_opts=None, _is_alias_mode=True)

    assert build_ssh_opts(ssh_client) == "-o StrictHostKeyChecking=no"


def test_build_ssh_opts_explicit_mode():
    ssh_client = SimpleNamespace(ssh_base_opts=None, _is_alias_mode=False, key_path="/tmp/test_key", port=2222)
...
def test_build_ssh_opts_reuses_base_opts_from_ssh_client():
    ssh_client = SimpleNamespace(ssh_base_opts=["-o", "StrictHostKeyChecking=no", "-o", "ControlMaster=auto"], key_path="/tmp/key", port=3333)
