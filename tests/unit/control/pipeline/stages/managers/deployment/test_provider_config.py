...
def test_get_provider_training_cfg_known_provider_returns_dict():
    ...
    # Post-Phase-B: ``docker_image`` was removed from the schema; the
    # canonical training block fields are ``workspace_path`` (required)
    # + the cleanup/execution defaults from SingleNodeTrainingConfig.
    assert training["workspace_path"] == "/tmp/workspace"
...
def test_get_single_node_training_cfg_returns_single_node_training():
    ...
    # Post-Phase-B: ``docker_image`` is no longer a config field; assert
    # against ``workspace_path`` which is the canonical required field.
    assert training["workspace_path"] == "/tmp/workspace"
