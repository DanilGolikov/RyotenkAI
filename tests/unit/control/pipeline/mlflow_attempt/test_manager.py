...
from ryotenkai_control.pipeline.mlflow_attempt.manager import MLflowAttemptManager
...
def _build_config() -> MagicMock:
    config = MagicMock()
    config.integrations.mlflow.tracking_uri = "http://localhost:5002"
    config.integrations.mlflow.local_tracking_uri = None
    config.integrations.mlflow.ca_bundle_path = None
    config.integrations.mlflow.system_metrics_callback_enabled = False
    return config
...
def test_setup_for_attempt_opens_root_and_attempt_runs(manager_under_test: MLflowAttemptManager) -> None:
    ...
    with patch("ryotenkai_pod.trainer.managers.mlflow_manager.MLflowManager", return_value=mgr):
        context: dict = {}
...
def test_open_existing_root_run_raises_when_no_manager(
    ...
    from ryotenkai_control.pipeline.mlflow_attempt.manager import MLflowManagerNotInitializedError
