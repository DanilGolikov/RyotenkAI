...
def test_cli_config_validate_valid_config(tmp_path: Path, cli_runner: CliRunner, monkeypatch) -> None:
    ...
    ``ryotenkai_control.api.services.config_service.validate_config``
    (no longer raw ``load_config``). The loader symbol lives in
    ``ryotenkai_shared.config.loader``; patch it there.
    """
...
def test_cli_config_validate_missing_hf_token(tmp_path: Path, cli_runner: CliRunner) -> None:
    """``config validate`` exits non-zero and mentions HF_TOKEN when missing.

    Post-Phase-B: see ``test_cli_config_validate_valid_config`` for the
    new patch target rationale.
    """
    import os
    from unittest.mock import MagicMock, patch
