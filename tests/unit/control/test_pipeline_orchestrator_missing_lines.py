...
def _mk_config() -> MagicMock:
    from tests._fakes.dataset_source import make_dataset_local
    ...
    # Post-Phase-B: production reads ``ds.source`` (typed) — supply a
    # real DatasetSourceLocal via the helper which also exposes the
    # legacy ``source_local`` / ``get_source_type`` accessors for any
    # remaining legacy code paths in this test file.
    ds = make_dataset_local(train="data/train.jsonl", eval_path=None)
    ds.adapter_type = "chat"
...
class TestPrintSummaryCleanupAndMetricsCollectionMissingLines:
    def test_print_summary_branches(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        from tests._fakes.dataset_source import make_dataset_hf
        ...
        hf_ds = make_dataset_hf(train_id="train", eval_id="eval")
        hf_ds.adapter_type = None
        ...
        # Case 2: dataset source not a recognised type (None) → "not configured"
        none_ds = SimpleNamespace(source=None, adapter_type=None)
        cfg.get_primary_dataset.return_value = none_ds
        orch._print_summary()
