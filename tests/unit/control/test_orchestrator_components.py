...
class TestDatasetLoader:
    ...
    def test_load_for_phase_file_not_found(self, mock_config):
        ...
        Post-Phase-B: ``DatasetLoader.load_for_phase`` reads
        ``dataset_config.source`` (a typed ``DatasetSourceLocal`` /
        ``DatasetSourceHF`` discriminated union) and routes via
        ``isinstance`` to ``_load_local_datasets`` /
        ``_load_hf_datasets``. The earlier ``train_path`` attribute
        is gone. Build a real ``DatasetSourceLocal`` so the production
        path reaches its file-not-found branch.
        """
        from ryotenkai_shared.config import DatasetLocalPaths, DatasetSourceLocal

        dataset_cfg = mock_config.get_dataset_for_strategy.return_value
        dataset_cfg.source = DatasetSourceLocal(
            local_paths=DatasetLocalPaths(train="/nonexistent.jsonl", eval=None),
        )
