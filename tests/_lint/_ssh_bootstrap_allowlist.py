...
Kept in its own module (not inline in the sentinel test) so changes
require an explicit PR review against this file with a written
justification (RP22 mitigation — allowlist staleness).

Each entry MUST carry a ``# why`` comment explaining the
bootstrap-or-out-of-scope rationale. The reviewer's job is to push
back when an SSH addition isn't a real bootstrap or pod→Mac data-pull
concern.
