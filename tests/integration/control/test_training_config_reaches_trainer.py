...
# pytestmark removed 2026-05-12 (xfail audit). Inline YAML fixtures
# migrated to typed ``source`` / ``adapter`` / ``engine`` discriminated
# unions; ``_FakeStrategy`` gained ``prepare_prompts_for_chat_template``.
...
class _FakeStrategy:
    ...
    def prepare_prompts_for_chat_template(self, train_dataset, eval_dataset, tokenizer):
        # No-op passthrough — mirrors TrainingStrategy default. Required by
        # TrainerFactory.create() since the post-packagization hook contract.
