"""CLI ↔ API payload-equality contract.

Greenfield migration of the per-pair payload-equality assertions from
``packages/control/tests/contract/test_cli_api_parity.py``. Complements
the reachability-only gate in :mod:`tests.contract.cli_api.test_parity`
with the strong property:

   The CLI reads ``pipeline_state.json`` directly; the API goes through
   services + a process-local cache. Both paths must emit the same
   payload modulo cosmetic differences (timestamp precision, list
   ordering, path types).

The suite is intentionally small (one test per pair) — the goal is
contract enforcement, not exhaustive coverage. Per-command edge cases
belong in component / integration tests that exercise each side
independently.
"""
...
from tests.contract.cli_api._normalize import normalise
...
def _seed_run(runs_dir: Path, run_id: str = "run_parity") -> Path:
    """Materialise a minimal completed-run directory the CLI + API can
    read."""
...
def test_runs_ls_matches_api_runs_list(
    parity_runs_dir: Path,
    parity_cli_runner: CliRunner,
    parity_cli_app,
    parity_api_client: TestClient,
) -> None:
    _seed_run(parity_runs_dir, "run_alpha")
    _seed_run(parity_runs_dir, "run_beta")

    cli_result = parity_cli_runner.invoke(
        parity_cli_app, ["-o", "json", "runs", "ls", str(parity_runs_dir)],
    )
    assert cli_result.exit_code == 0, cli_result.stdout
    cli_payload = json.loads(cli_result.stdout)

    api_response = parity_api_client.get("/api/v1/runs")
    ...
    api_rows = [
        r for rows in api_payload.get("groups", {}).values() for r in rows
    ]
...
def test_runs_inspect_matches_api_get_run(
    ...
    run_dir = _seed_run(parity_runs_dir, "run_gamma")

    cli_result = parity_cli_runner.invoke(
        parity_cli_app, ["-o", "json", "runs", "inspect", str(run_dir)],
    )
    assert cli_result.exit_code == 0, cli_result.stdout
    cli_payload = json.loads(cli_result.stdout)

    api_response = parity_api_client.get(f"/api/v1/runs/{run_dir.name}")
    assert api_response.status_code == 200
    api_payload = api_response.json()

    # Compare a stable subset that both sides are expected to expose
    # identically. Field renames (CLI ``run_id`` ↔ API
    # ``logical_run_id``) are bridged here to stay declarative.
    cli_subset = {
...
def test_preset_ls_matches_api_presets(
    ...
    cli_result = parity_cli_runner.invoke(
        parity_cli_app, ["-o", "json", "preset", "ls"],
    )
    assert cli_result.exit_code == 0, cli_result.stdout
    cli_payload = json.loads(cli_result.stdout)

    api_response = parity_api_client.get("/api/v1/config/presets")
    assert api_response.status_code == 200
    api_payload = api_response.json()
