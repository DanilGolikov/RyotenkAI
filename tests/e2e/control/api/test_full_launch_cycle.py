...
def _fake_spawn_writes_state(run_dir: Path):
    ...
    Signature matches the current ``spawn_launch`` surface (returns
    ``(pid, command, launcher_log)``).
    """
...
def test_launch_then_poll_state_reflects_completion(tmp_path: Path, monkeypatch) -> None:
    ...
    # Phase B packagization renamed spawn_launch_detached -> spawn_launch.
    # launch_service imports it as ``spawn_launch`` from
    # ryotenkai_control.pipeline.launch.
    monkeypatch.setattr(launch_service, "spawn_launch", _fake_spawn_writes_state(target_dir))
