      ...
      <Card>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-surface-2 border border-line-2 rounded-md px-3 py-2 text-sm font-mono focus:border-burgundy-400 focus:outline-none"
          />
          <button
            type="button"
        ...
        {validateMut.error && (
          <div className="mt-3 text-xs text-status-err bg-status-err/10 border border-status-err/30 px-3 py-2 rounded">
            {(validateMut.error as Error).message}
          </div>
        )}
        {validation && (
          <div className="mt-4 space-y-1">
            <div className={`text-xs font-medium ${validation.ok ? 'text-status-ok' : 'text-status-err'}`}>
              ...
              {validation.checks.map((check, idx) => (
                <li
                  ...
                  className={
                    ...
                    check.status === 'warn' ? 'text-status-warn' :
                    'text-status-err'
                  }
                >
                  [{check.status.toUpperCase()}] {check.label}
                  {check.detail && <span className="text-ink-mute"> — {check.detail}</span>}
                </li>
              ))}
            </ul>
          </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block">
            <span className="text-2xs uppercase tracking-wider text-ink-mute">Run id</span>
            <input
              value={runId}
              onChange={(event) => setRunId(event.target.value)}
              placeholder="auto"
              className="w-full mt-1 bg-surface-2 border border-line-2 rounded-md px-3 py-2 text-sm font-mono focus:border-burgundy-400 focus:outline-none"
            />
          </label>
          <label className="block">
            <span className="text-2xs uppercase tracking-wider text-ink-mute">Subgroup</span>
            <input
              value={subgroup}
