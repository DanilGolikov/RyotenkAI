...
import { useCreateIntegration, useIntegrationTypes } from '../api/hooks/useIntegrations'
...
export function NewIntegrationModal({
  ...
  open: boolean
  onClose: () => void
  /** Pre-select the integration type when the modal is opened from a
   *  type-specific tab (e.g. the MLflow tab pre-selects "mlflow"). */
  initialType?: string
}) {
  ...
  useEffect(() => {
    if (!open) {
      ...
      if (initialType) {
        setType(initialType)
      } else if (typesQuery.data && typesQuery.data.types.length > 0) {
        setType(typesQuery.data.types[0].id)
      }
    }
  }, [open, typesQuery.data, initialType])
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      onClose()
      navigate(`/settings/integrations/${summary.id}`)
    } catch {
      /* error shown below */
    }
  }
    ...
    >
      ...
      >
        <div className="px-5 py-4 bg-surface-2 border-b border-line-1">
          <div className="text-sm font-semibold">New integration</div>
          <div className="text-2xs text-ink-3">
            Reusable integration (HuggingFace / MLflow). Versioned on every save.
            Token is stored encrypted after you add it in the Token tab.
          </div>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <label className="block">
            ...
            <input
              ...
              className="w-full rounded-md bg-surface-2 border border-line-1 px-3 py-2 text-sm focus:outline-none focus:border-brand"
              placeholder={type === 'mlflow' ? 'MLflow prod' : 'HF account'}
            />
          </label>

          <label className="block">
            <div className="text-2xs text-ink-3 mb-1">
              Integration id <span className="text-ink-4">(slug)</span>
            </div>
            <input
              value={derivedId}
              onChange={(e) => {
          ...
          <label className="block">
            ...
            <textarea
              ...
              rows={2}
              className="w-full rounded-md bg-surface-2 border border-line-1 px-3 py-2 text-sm focus:outline-none focus:border-brand"
              placeholder="Where is this integration used?"
            />
          </label>
