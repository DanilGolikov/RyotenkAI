...
| `code-syncer-attr-drift` | `tests/unit/control/pipeline/stages/managers/deployment/test_code_syncer.py` (7 tests) | `CodeSyncer` class attributes/constructor signature drifted during Phase B packagization; tests construct fixtures with legacy kwargs. | deployment stack owner | Read current `CodeSyncer.__init__` signature; build a thin factory `tests/_fakes/code_syncer.py::make_code_syncer(**overrides)`; rewrite tests against it. ~30-45 min once the factory exists. |
...
| `launch-service-resume-api-drift` | `tests/unit/control/api/services/test_launch_service_resume_pod.py::test_missing_api_key_returns_skipped` | Provider registry now requires `api_key` kwarg for `create_resume_provider` factories; test still exercises legacy no-arg path. | api stack owner | Construct provider with explicit `api_key=None`; adjust assertions to match new registry shape (likely a "SKIPPED" outcome at a different layer). |
| `resume-service-signature-drift` | `tests/unit/control/pipeline/launch/test_resume_service.py::test_default_resolver_uses_env_for_runpod` | `RunPodProvider.from_resume_metadata()` signature changed (additional kwarg required). | resume service stack owner | Check current `from_resume_metadata` signature; update test invocation. |
